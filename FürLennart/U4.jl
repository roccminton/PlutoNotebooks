...
# ╔═╡ 25ca8324-e8cc-4e97-bb22-151f57d132f9
md"#### Aufgabe 4.1"

# ╔═╡ 74b8dc6c-3e29-462f-9316-c518168bfd00
md"""
a) One can do the calculations and find that the Taylor series of sin and cosine are given by

$$f(\tilde{x}) = \sum\limits_{k=0}^\infty (-1)^k \frac{\tilde{x}^{2k+1}}{(2k+1)!}$$
$$g(\tilde{x}) = \sum\limits_{k=0}^\infty (-1)^k \frac{\tilde{x}^{2k}}{(2k)!}$$
"""

# ╔═╡ d4aca076-49a5-4e8d-ba32-efbeaf9f391a
md"b)"

# ╔═╡ 4f3f9934-b9db-4da7-8dd9-4d8d22bae51a
begin
	#Gives the k-th summand of the taylor series of the function f, that can be or cos or sin around x at y
	_T(f,k,x,y) = 1/factorial(k) * f(x+k*π/2) * (y-x)^k
	#Gives the taylor approximation of the function f ∈ [sin,cos], at y around x of the n-th order
	T(f,n,x,y) = sum(_T(f,k,x,y) for k in 0:n)
end

# ╔═╡ f39204b8-cb09-4cc2-9eec-8f4836e6c0a3
begin
	#Define the functions f and g from 4.1b)
	f(x,n) = T(sin,n,0,x)
	g(x,n) = T(cos,n,0,x)
end
...
let
	...
	pᵢ = plot(
			X,sin.(X),label="sin",
			ylim=(-2,2),lw=2,
			xticks = ([0,π/2,π,3π/2,2π],[L"0",L"\frac{\pi}{2}",L"\pi",L"\frac{3\pi}{2}",L"2\pi"]),
			title = "Sinus and its Taylor approximations"
	)
	for n in N
		plot!(
			pᵢ,X,f.(X,n),
			label="$n-" * (n==1 ? "st" : (n == 3 ? "rd" : "th")) * " order"
		)
	end
	#vline!([0.05,π/2,π],lw=1,color=:black,linestyle=:dash,label="")
	pᵢ
end
...
let
	...
	pᵢᵢ = plot(
			...
			ylim=(-2,2),lw=2,
			xticks = ([0,π/2,π,3π/2,2π],[L"0",L"\frac{\pi}{2}",L"\pi",L"\frac{3\pi}{2}",L"2\pi"]),
			title = "Cosinus and its Taylor approximations"
	)
	for n in N
		plot!(pᵢᵢ,X,g.(X,n),label="$n-" * (n==2 ? "nd" : "th") * " order")
	end
	#vline!([0.05,π/2,π],lw=1,color=:black,linestyle=:dash,label="")
	pᵢᵢ
end
...
md"Absolute error for approximation of sin"

# ╔═╡ 88f00ade-14a3-4bf2-8c4a-6ef1c5de3c3c
let
	X = [0.05,π/2,π]
	N = 1:2:11
	data = hcat(N,[abs.(sin.(x) .- f.(x,N)) for x in X]...)
	header = ["n","x=0.05","x=π/2","x=π"]

	pretty_table(data;header=header)
end

# ╔═╡ db3ce41f-ee74-46cf-8418-d9c61c73447f
md"Absolute error for approximation of cos"

# ╔═╡ 7764627b-59d3-40a3-8724-62d5c4d62479
let
	X = [0.05,π/2,π]
	N = 1:2:11
	data = hcat(N,[abs.(cos.(x) .- g.(x,N)) for x in X]...)
	header = ["n","x=0.05","x=π/2","x=π"]

	pretty_table(data;header=header)
end

# ╔═╡ 57ee3cee-7d58-4140-98f0-09abb6873848
md"d)"

# ╔═╡ e420ce0c-c28f-4613-9e2f-a077d26cca8a
F(x,y) = sin(x) + cos(y)

# ╔═╡ ac98e874-ed31-483e-8efb-1296d66f446c
begin
	X = -π:0.01:π
	ticks = (
			[-π,-π/2,0,π/2,π],
	...
	original = surface(
		X,X,[F(x,y) for x in X, y in X],
		xticks = ticks, yticks = ticks
	)
end

# ╔═╡ 70c00131-9ed6-4e45-bfcc-8a458b9f31ab
#Gives the (i,j)-th patial derivative of F at (0,0)
function dF(i,j)
...
begin
	#Gives the "k-th" summand of the Taylor approximation of F around (0,0) at (x,y). Here "k-th" means all the sum of all the index pairs (i,j) with i+j = k
	_TF(k,x,y) = sum( 1/(factorial(i)*factorial(k-i)) * dF(i,k-i) * x^i * y^(k-i) for i in 0:k )
	#Gives the n-th order Taylor approximation of F around (0,0) at (x,y)
	TF(n,x,y) = sum( _TF(k,x,y) for k in 0:n)
end

# ╔═╡ 39617d33-ea0b-41b2-8131-42a4a8862d0b
#helping function - nur spielerei...
function order(n)
	if n == 1
		o =  "1-st"
	elseif n == 2
...
let
	#setup vector with first plot
	🌻 = [original]
	#add title to first plot
	plot!(original,title="original")

	#plot all the approximations and add to vector
	for n in 1:11
		p = surface(X,X,[TF(n,x,y) for x ∈ X, y ∈ X], title = order(n))
		push!(🌻,p)
	end
	#plot all the plots
	plot(
		🌻...,layout=(3,4),zlim=(-2,2),titlefontsize = 7,
		ticks=false,axis=false,colorbar=false
	)
end
