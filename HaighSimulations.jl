...
begin
	load(X) = sum(number_mutations(X,i-1)*X[i] for i in 2:length(X))/(sum(X)-X[1])
	number_mutations(X,i) = X[1]+i-1
end

# ╔═╡ 0a49f851-4a58-4545-877a-f452bb2e47bf
function eq_initial(N,λ,slots)
...
extimes = [findextime(Xs,i) for i in 2:findfirst(!iszero,Xs[t_end,:])-1];

# ╔═╡ 5d0ee2af-d22b-49a8-b071-2e631c181e00
begin
	pt = plot(ML,ylabel="Mutation Load",xlabel="Time",legend=false)
	vline!(pt,[t])
	if showtimes
		vline!(pt,extimes)
	end
	br = bar(Xs[t,2:slots+1],ylim=(0,1.5*10^4),xlabel="Classes",legend=false)
...
begin
	...
	Xs_mean = mean(runHaigh(eq_initial(N_mean,λ_mean,slots_mean),s_mean,λ_mean,t_end_mean) for _ in 1:n_runs)
end;
...
begin
	...
	for t in Ts
		plot!(pl, 0:slots_mean-1,Xs_mean[t+1,2:51], 
			label="t=$t",
			xlim=(0,30),
