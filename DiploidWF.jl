...
md"# Diploid Wright Fisher"

# ╔═╡ 0baf862c-58d4-4d20-876e-912914e902bf
md""" The goal of this worksheet is to provide an easier method to simulate a diploid Wright Fisher Model. For simplicity we look at one Gene only that can have two allels $a$ and $A$.

First we create an "$\textbf{individual based}$" model where we sample diploid individuals. And for comparison we sample from an "$\textbf{allel based}$" model, where we sample alleles and recreate the individual composition via the Hardy-Weinberg equilibrium.
"""

# ╔═╡ e598fc50-ff9c-4f7b-87bc-a19fcc375cb2
md"""The population state in the individual based model is given by 

$\mathbf{X} = [X_{aa},X_{aA},X_{AA}]$ 

whereas the population state in the allel based model is giben by 
...
begin
	couples(X,l) = [(i==j ? 1 : 2)*X[i]*X[j] for i in 1:l for j in i:l]
	couples(X) = couples(X,length(X))
end

# ╔═╡ 5983504b-3bdf-4efb-ba94-44bf5efe9074
begin
	function run_wright_fisher(X₀,t_end,step!)
		l = length(X₀)
		history = zeros(l,t_end)
		Xₜ = copy(X₀)
		N = round(Integer,sum(X₀))
		for t in 1:t_end
			#safe current status
			history[:,t] .= Xₜ
	...
	function haploid_WF_step!(Xₜ,t,N)
		#sample new population
		Xₜ .= rand(Multinomial(N,Xₜ ./ N))
	end

	function diploid_WF_step!(Xₜ,t,N)
		#generate couples
		C = rand(Multinomial(N,couples(Xₜ ./ N)))
		#aa from aa*aA
		f_aa = rand(Binomial(C[2],1/2))
		#aA from aA*AA
		f_aA = rand(Binomial(C[5],1/2))
		#aa|aA|AA from aA*aA
		f = rand(Multinomial(C[4],[1/4,1/2,1/4]))
		#sample new population
		Xₜ[1] = C[1] + f_aa + f[1]
		Xₜ[2] = C[3] + C[2]-f_aa + f_aA + f[2]
		Xₜ[3] = C[6] + C[5]-f_aA + f[3]
	end

	function multinomial_WF_step!(Xₜ,t,N)
		C = couples(Xₜ)
		p = [
			C[1] + 1/2 * C[2] + 1/4 * C[4],
			C[3] + 1/2 * C[2] + 1/2 * C[4] + 1/2 * C[5],
			C[6] + 1/2 * C[5] + 1/4 * C[4]
		]
		Xₜ .= rand(Multinomial(N,p./N^2))
	end
end
...
begin
	function plot_WF(X)
		...
		plot([X[i,:] for i in 1:length(X[:,1])],label=label)
	end
end
...
begin
	function altoind(a,A)
		n = a+A ; p = a/n; q = A/n
		return [p^2,2*p*q,q^2] .* (n/2)
	end
	altoind(Y::Array) = altoind(Y...)
	indtoal(aa,aA,AA) = [2*aa+aA,2*AA+aA]
	indtoal(X::Vector) = indtoal(X...)

	function convertmatrix(func,M)
		nM = zeros(length(func(M[:,1])),size(M)[2])
...
X = run_wright_fisher(X₀,t_end,multinomial_WF_step!)
