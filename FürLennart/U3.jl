...
# ╔═╡ c0e18196-3177-4983-943e-82b296f6f7a6
using Plots

# ╔═╡ ab5a0bbe-4292-4688-94ec-9c54e26405aa
using PrettyTables

# ╔═╡ 6a80f361-92f3-4468-a5d7-7f4a279c2ed5
using LaTeXStrings

# ╔═╡ 2fce3b3d-6531-4cc5-a962-caada0898c79
md"LaTeXStrings is used to make nicer looking ticks in the plot for exercise 3"
...
# ╔═╡ 5ec080a7-089a-41b0-a81e-305ae8888eb8
md"#### Aufgabe 3"

# ╔═╡ 8ccabeae-df96-4509-b5ad-a30542996dba
md"Define the function that we want to iterate"

# ╔═╡ 119c3a28-54b6-44b6-b558-f6c09cd3e25b
f(x) = sqrt(2*(1-sqrt(1-x^2/4)))

# ╔═╡ b652aae1-76ad-428c-ab2c-aa60c1ad3e45
md"Define the function for the area of the n-Eck. Note, that we do not need to divide $n\cdot s_n$ by halfe. $\pi$ is actually the area of the whole unit circle, not of half of it..."

# ╔═╡ 19717fee-6361-4589-aeb4-ccf10615d5fa
A(n,s_n) = (n*s_n)

# ╔═╡ 96a3a672-b150-4285-8f9c-eb1355c24622
md"Do the iteration and gradually build up the $s_n$'s"

# ╔═╡ 492dc559-d457-4501-9244-21c5e0010213
begin
	s = [sqrt(2)]
...
# ╔═╡ 4d400d22-6b07-4d72-9d97-bf9bda276e5c
md"Finally calculate the areas and the error below"

# ╔═╡ 0c5a06ef-c614-42e2-ad2a-bbb4563d87e5
Aₙ = [A(2^i,s[i]) for i in 1:30];

# ╔═╡ 61b94943-e4ba-4a6c-ba62-dc26fb37dcf6
👎 = abs.(Aₙ .- π);

# ╔═╡ d13d6fe7-1d37-4e93-88fa-468d55c2a1d8
md"If we look at the plot"

# ╔═╡ 008c83b1-25fa-4705-80a5-5b4d30b5217a
begin
	plot(
		1:30,Aₙ,
		ylim=π .+ [-0.01,0.01],label="Aₙ",xlim=[0,31],
		xlabel = L"n"
	)
	xticks!(collect(0:5:30),[L"2^{%$n}" for n in 0:5:30])
	hline!([π],label="π")
end

# ╔═╡ 3bcc790d-b975-4f1f-b74d-8d97b286d02d
md"or the table"

# ╔═╡ 9c89c729-5b14-4b90-a043-574ce7e1b987
begin
	data = hcat(2 .^ 0:30, s, Aₙ, 👎)
	header = ["n","sₙ","Aₙ","| Aₙ-π |"]
	pretty_table(data,header=header)
end

# ╔═╡ 424c894a-58a9-49a8-98ba-0e35e4a85d38
md"Then we see that we come pretty close to $\pi$ with $A_n$, but in the end the computer cant handle the small $s_n$'s any more and he fucks up." 
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "4be5f6319a0baeab4061ecacb29e549a41f60307"
